...
export const saveRentalData = async (userId: string, propRentalData: any) => {
  try {
    const response = await axios.post(
      `http://localhost:4000/api/prop-rental-details`,
      {
        userId,
        propRentalData,
      },
...
export const getRentalData = async (userId: string) => {
  try {
    const response = await axios.get(
      `http://localhost:4000/api/prop-rental-details/${userId}`,
      {
        headers: {
    ...
    if (response.status === 200) {
      ...
      return {
        success: false,
        message: response.data.message || "Failed to fetch rental details.",
      };
    }
  } catch (error: any) {
    ...
    return {
      success: false,
      message: error.message || "Error fetching rental data.",
    };
  }
};
