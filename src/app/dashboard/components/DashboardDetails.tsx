...
const DashboardDetails: React.FC<IntroProps> = ({ user, venueDetails }) => {
  ...
  const photo_video = Object.entries(venueDetails).filter(([key]) =>
    ...
    return (
        <div className=" flex gap-8 flex-col rounded-xl bg-white p-3 md:p-6">
            ...
            <div>
                {selected === 0 &&
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8 ">
                        <div className="flex flex-col gap-4 border-2 p-5 rounded-[20px]">
                            <div className='flex flex-col gap-2 text-gray-700'>
                                <div className='text-base font-normal'>GSTIN:</div>
                                <div className='font-bold text-sm '>{user.businessDetails.gstin}</div>
                            </div>
                        </div>
                        <div className="flex flex-col gap-4 border-2 p-5 rounded-[20px]">
                            <div className='flex flex-col gap-2 text-gray-700'>
                                <div className='text-base font-normal'>Team Size:</div>
                                <div className='font-bold text-sm '>{user.businessDetails.teamsize} Persons</div>
                            </div>
                        </div>
                        <div className="flex flex-col gap-4 border-2 p-5 rounded-[20px]">
                            <div className='flex flex-col gap-2 text-gray-700'>
                                <div className='text-base font-normal'>Buisness Address:</div>
                                <div className='font-bold text-sm '>{user.businessDetails.businessAddress}</div>
                            </div>
                        </div>
                        <div className="flex flex-col gap-4 border-2 p-5 rounded-[20px]">
                            <div className='flex flex-col gap-2 text-gray-700'>
                                <div className='text-base font-normal'>Pincode:</div>
                                <div className='font-bold text-sm '>{user.businessDetails.pinCode}</div>
                            </div>
                        </div>
                        <div className="flex flex-col gap-4 border-2 p-5 rounded-[20px]">
                            <div className='flex flex-col gap-2 text-gray-700'>
                                <div className='text-base font-normal'>Operational Cities:</div>
                                <div className='font-bold text-sm '>{Array.isArray(user.businessDetails.cities) ? user.businessDetails.cities.join(", ") : user.businessDetails.cities}</div>
                            </div>
                        </div>
                        <div className="flex flex-col gap-4 border-2 p-5 rounded-[20px]">
                            <div className='flex flex-col gap-2 text-gray-700'>
                                <div className='text-base font-normal'>Years in Operation:</div>
                                <div className='font-bold text-sm '>{user.businessDetails.years} Years</div>
                            </div>
                        </div>
                        <div className="flex flex-col gap-4 border-2 p-5 rounded-[20px]">
                            <div className='flex flex-col gap-2 text-gray-700'>
                                <div className='text-base font-normal'>Annual Revenue:</div>
                                <div className='font-bold text-sm '>{user.businessDetails.annualrevenue} Lakhs</div>
                            </div>
                        </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8 ">
                        {booleanFields.map(([key, value]) => (
                            <div key={key} className="flex flex-col gap-4 border-2 p-5 rounded-[20px]">
                                <div className='flex flex-col gap-2 text-gray-700'>
                                    <div className='text-base font-normal'>Do you provide {key.charAt(0).toUpperCase() + key.slice(1)}?</div>
                                    <div className='font-bold text-sm '>{value ? "Yes" : "No"}</div>
                                </div>
                            </div>
                        ))}
                            ...
                            <div key={key} className="flex flex-col gap-4  px-5 rounded-[20px]">
                                <div className='flex flex-col gap-2 text-gray-700'>
                                    ...
                                    {Array.isArray(value) && value.length > 0 ? (
                                        <div className='font-bold text-sm flex flex-wrap gap-2 '>
