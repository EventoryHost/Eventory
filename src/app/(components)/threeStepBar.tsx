...
const ThreeStepBar = ({ currentStep }: { currentStep: number }, onStepChange: (step: number) => void) => {
  return (
      ...
      <div className="flex">
        {[1, 2, 3].map((step) => (
          <button
            ...
            className={`ml-2 h-2 w-14 rounded-full ${currentStep >= step ? "bg-[#252775]" : "bg-gray-300"} opacity-100`}
          />
        ))}
      </div>
    </div>
