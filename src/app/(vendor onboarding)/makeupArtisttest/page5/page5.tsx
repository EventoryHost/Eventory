...
const Page: React.FC<Page5Props> = ({ formState, makeupTypes, category, makeupArtists_individual, hourlyPackage, dealPackage, ratesbyWorker , setCurrentPage, onsiteMakeup }) => {
        ...
        <div className="flex h-full min-h-[calc(100vh-5.2rem)] w-full flex-col overflow-hidden lg:flex-row">
            ...
            <div className="scroll-touch flex max-h-[calc(100vh-5.2rem)] min-w-[70%] flex-col items-center gap-9 overflow-y-scroll bg-[#F7F6F9] p-2 md:p-[1rem]">
                <div className="flex flex-col gap-7 rounded-xl bg-white p-3 xs:min-w-[90%] md:p-6">
                    <h1 className="text-3xl font-semibold">{formState.artistName} / Make-up Artist</h1>
                    ...
                    <div className="mx-8 font-semibold text-xl">
                        {category == "Individual" ? "Individual" : category == "Group" ? "Group" : "Organisation"}
                    </div>

                    {formState.category == "Individual" && <div className="mx-8 mt-2 flex gap-16">
                        <div className="flex w-1/2 flex-col">
                            <span className="text-xl">Total Members</span>
                            <span className="mt-4 font-semibold">{formState.organisationMembers}</span>
                        </div>
                    </div>}
                    ...
                    <div className="mx-8 mt-6 flex gap-16">
                        <div className="flex w-1/2 flex-col">
                            ...
                            <span className="mt-4 font-semibold">{formState.artistDescription}</span>
                        </div>
                        <div className="flex w-1/2 flex-col">
                            <span className="text-xl">PortFolio</span>
                            <span className="mt-4 font-semibold">{(typeof formState.portfolioUrls === 'string' ? formState.portfolioUrls : formState.portfolioUrls.name) || "No File Uploaded"}</span>
                        </div>
                    </div>
                    <div className="mx-8 mt-6 flex flex-col gap-10">
                        <div className="flex w-1/2 flex-col">
                            <span className="text-xl">Which Type(s) of Make Up Artist You are?</span>
                        </div>
                        <div className="flex w-full flex-col">
                            <div className="flex flex-wrap gap-4">
                                {makeupArtists_individual.length > 0 ? (
                                    makeupArtists_individual.map((entry, index) => (
                                        <div key={index} className="bg-[#e6e6fa] px-4 py-2 rounded-full text-center">
                                            <span>{entry}</span>
                                        </div>
                                    ))
                                ) : (
                                    <div className="bg-[#e6e6fa] px-4 py-2 rounded-full text-center">No Options Selected</div>
                                )}
                            </div>
                        </div>
                        ...
                        <div className="mx-8 mt-2 flex gap-16">
                            <div className="flex w-1/2 flex-col">
                                ...
                                <span className=" font-semibold">{(typeof formState.cancellationPolicy === 'string' ? formState.cancellationPolicy : formState.cancellationPolicy.name) || "No file uploaded"}</span>
                            </div>
                            <div className="flex w-1/2 flex-col">
                                <span className="text-xl">Terms and Conditions</span>
                                <span className=" font-semibold">{(typeof formState.termsAndConditions === 'string' ? formState.termsAndConditions : formState.termsAndConditions.name) || "No File Uploaded"}</span>
                            </div>
                        </div>
                        ...
                        <div className="mx-8 mt-2 flex gap-16 flex-wrap">
                        ...
                        <div className="mx-8 mt-2 flex gap-16">
                            <div className="flex w-1/2 flex-col">
                                ...
                                <span className="mt-2 font-semibold">{onsiteMakeup ? "Available" : "Unavailable"}</span>
                            </div>
                        </div>

                        <div className="mx-8 mt-2 flex gap-4 flex-wrap">
                            <div className="flex w-full flex-col">
                                <span className="text-xl mb-2">Specialization Makeups</span>
                                <div className="flex flex-wrap gap-4">
