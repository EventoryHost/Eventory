...
interface FilterSectionProps {
    ...
    onFilterChange: (sectionKey: string, filter: string, checked: boolean) => void;  // Notify parent of changes
    selectionType: 'single' | 'multiple';  // Specify whether single or multiple selection is allowed
}

const FilterSection: React.FC<FilterSectionProps> = ({ title, filters, sectionKey, selectedFilters, onFilterChange, selectionType }) => {
            ...
            {isOpen && (
                <div className='my-2 p-2'>
                    ...
                    {filters.map((filter, index) => (
                        <div key={`${sectionKey}-${index}`} className='flex items-center my-2'>
                            <div
                                onClick={() => {
                            ...
                            <label htmlFor={`${sectionKey}-checkbox-${index}`}>{filter}</label>
                        </div>
                    ))}
                </div>
            )}
