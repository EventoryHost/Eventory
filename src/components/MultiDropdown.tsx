...
const MultipleDropdown: React.FC<DropdownProps> = ({
    options,
    onSelect,
                ...
                <div className="flex flex-wrap gap-2">
                    {selectedOptions.map((option) => (
                        <span
                            ...
                            className="rounded-lg bg-indigo-100 h-10 flex justify-around items-center gap-2 px-3 py-1 text-sm font-medium text-indigo-700"
                        >
                            {option.label}
                            <span className="bg-white w-5 flex justify-center items-center text-center h-5 rounded-full">
                                <svg width="13" height="13" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                                    <path d="M2 2L14 14M2 14L14 2" stroke="#2B3F6C" stroke-width="1.33333" stroke-linecap="round" stroke-linejoin="round" />
                                </svg>
                            </span>
                        </span>
                    ))}
                ...
                <div className="absolute z-10 mt-1 w-full rounded-md bg-white shadow-lg">
                    <ul className="max-h-60 overflow-auto rounded-md py-1 text-base focus:outline-none sm:text-sm">
                        {options.map((option) => (
                            <li
                                key={option.value}
                                className={`relative flex cursor-pointer select-none items-center gap-1 py-2 pl-3 pr-9 ${selectedOptions.some((selected) => selected.value === option.value)
                                    ? "text-black"
                                    : "text-gray-900"
                                    }`}
                                onClick={() => handleSelect(option)}
